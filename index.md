...
- [[de-id-scan-working-tree-not-git-diff]] — `correction` · de-identify-before-commit scans must include **untracked** files (`git grep --untracked …`, or scan *after* `git add`); a bare `git diff`/`git grep` sees only tracked files, so a brand-new untracked note still holding a confidential name passes as "clean".
