...
- **Vault exists** and is its own git repo (`git -C <vault> rev-parse --is-inside-work-tree`).
  By design there is **no** standing `base` remote — `/update-base` adds a dedicated
  `base-ephemeral` remote only for the fetch and removes it; a fork/custom base URL is
  persisted in `.agents/.base-url`. Don't "repair" a missing `base` remote by re-adding one.
...
- **Local-first**: everything works with no GitHub account. `update-base` pulls engine
  updates from the public base repo without auth (via an ephemeral `base-ephemeral` remote
  it adds for the fetch and then removes).
