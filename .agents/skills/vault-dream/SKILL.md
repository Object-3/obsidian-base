---
name: vault-dream
description: Consolidate this Obsidian knowledge base and fold durable learnings from recent agent sessions into it — a single "dream" pass that reads session transcripts since a watermark, captures reusable learnings into docs/knowledge/, dedupes/merges/re-indexes the vault, and hands the whole changeset off as a reviewable branch + pull request (never a write to main). Use when the user says "run the dream", "run /vault-dream", "consolidate the vault", "dream", "fold my sessions into the KB", "clean up and re-index the vault", "compound my recent sessions", or when the SessionStart nudge offers it (">= N unconsolidated sessions since your last dream"). Runs ONLY when working inside the vault repo itself (never when merely consuming the vault via the Obsidian MCP from another project). Composes the existing engine — dream-scan.sh (session discovery + a portable JSONL extractor), kw-compound, the researcher agents, stale-knowledge-checker, lint-vault.sh, /normalize-vault, and the compound-engineering session primitives when present — rather than reimplementing them. Never auto-merges, never deletes human-authored prose, de-identifies before writing to the tracked vault.
---
