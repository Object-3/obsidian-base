...
- The **5 hand-authored vault-engine skills** (`onboard`, `setup-vault`, `update-base`,
  `offboard`, `normalize-vault`, plus `install-skills` itself) are never mirrored — they
  aren't in the lock and only make sense inside a vault.
...
- **Precedence: personal > project.** Your user-scope copy shadows the vault's in-repo
  ...
  multiple vaults the mirror is **last-writer-wins** (`/install-skills --status` flags it).
